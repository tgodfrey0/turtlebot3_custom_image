...
2. At build time, `build.py` detects `robots/<robot_type>/` and copies it into
   the image at `/tmp/robot_plugin`.
3. The generic provisioner `scripts/50_robot_setup.sh` sources
   `/tmp/robot_plugin/setup.sh` (if present), which runs build-time setup.
...
```
robots/<robot_type>/
├── setup.sh              # Build-time setup (sourced by 50_robot_setup.sh)
└── <runtime_scripts>.sh  # First-boot scripts, copied into ~/setup_scripts
```

`setup.sh` runs **inside the chroot during image build** with full root access.
It should install packages, build workspaces, create marker files, enable
first-boot services, and copy any robot-specific runtime scripts into
